...
def run_import(get_variants):
    ...
    PRESTASHOP_API_KEY = os.environ.get(
        "PRESTASHOP_API_KEY", "RZY9E7L8AP5EPSMDZSXQ2SDJXZCEXBU4"
    )
    LANGUAGE_ID = os.environ.get("PRESTASHOP_LANGUAGE_ID")

    GATEWAY_BASE_URL = os.environ.get("GATEWAY_BASE_URL", "https://sma.dev.gwapi.eu")
    GATEWAY_SHOP_ID = os.environ.get(
        "GATEWAY_SHOP_ID", "a547de18-7a1d-450b-a57b-bbf7f177db84"
    )
    GATEWAY_SECRET = os.environ.get(
        "GATEWAY_SECRET", "OyB2YbwTVtRXuJv+VE4oJLVyGo8pf1XVibCk08lt4ys="
    )
    ...
    exporter = Gateway(
        GATEWAY_BASE_URL, GATEWAY_SHOP_ID, GATEWAY_SECRET, IMAGE_URL_PREFIX
    )
